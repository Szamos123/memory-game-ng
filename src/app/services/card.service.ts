...
export class CardService {
  ...
  private readonly apiKey =
    'NmCmxM05Yx4suPBh3ffIksxGRpXAAu14A86qtRQ7tgzZgXAnZQ7feLpH';
  ...
  fetchAndSetCards(perPage: number = 6): void {
    ...
    this.http
      .get<any>(
        `${this.apiUrl}?query=${randomQuery}&per_page=${perPage}&page=${randomPage}`,
        { headers }
      )
      .subscribe(
        (response) => {
          const imageUrls: string[] = response.photos.map(
